...
getBotton.addEventListener("click", async () => {
  ...
  if (name !== "" && limit !== 0) {
    url = `/characters/${name}/?limit=${limit}`;
  } else if (name !== "") {
    url = `/characters/${name}`;
  } else if (limit !== 0) {
  ...
  const result = await get(url).then((result) => {
...
postBotton.addEventListener("click", async () => {
  ...
  const result = await post(name, title, type).then((result) => {
    return createContent(result);
  });
...
async function put(name, title_value, type_value, target) {
  return await fetch(`/characters/${target}`, {
    method: "PUT",
    body: JSON.stringify({
      name,
      title_value,
...
putBotton.addEventListener("click", async () => {
  ...
  const result = await put(name, title_value, type_value, target).then(
    (result) => {
      return createContent(result);
    }
  );
...
deleteBotton.addEventListener("click", async () => {
  ...
  const result = await del(target).then((result) => {
    return createContent(result);
  });
